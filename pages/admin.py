...
def admin_dashboard_page() -> None:
    ...
    if not discord_id:
        ...
        return
    with ui.tabs(on_change=on_tab_change).style('width: 100%; max-width: margin: 0 auto;') as panels:
        panels.set_value(app.storage.user.get('admin_selected_tab', 'Schedule'))
        ui.tab('Schedule')
        ui.tab('Users')
        ui.tab('Tournaments')
        ui.tab('Settings')
    with ui.tab_panels(panels, value=app.storage.user.get('admin_selected_tab', 'Schedule')):
