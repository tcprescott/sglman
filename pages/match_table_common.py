...
class MatchTableView:
    ...
    def _setup_ui(self):
        self.show_upcoming_checkbox = ui.checkbox('Show only upcoming matches', value=True, on_change=self.refresh)
        with ui.row().style('width: 100%;'):
            if self.submit_match_callback:
                ui.button('Submit Match', on_click=self.submit_match_callback)
    ...
    async def refresh(self, *args, **kwargs):
        ...
        for m in all_matches:
            player_names = ', '.join([p.user.preferred_name for p in m.players])
            row = {
                'id': m.id,
    ...
    async def update_row_by_id(self, match_id):
        ...
        idx = next((i for i, row in enumerate(self.table.rows) if row.get('id') == match_id), None)
        if idx is None:
