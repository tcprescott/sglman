...
| 3 | Discord Events sync | [discord-events-sync.md](discord-events-sync.md) | Idempotent auto-sync of Discord Scheduled Events from Wizzrobe schedule data |
| 4 | SpeedGaming schedule ETL | [speedgaming-etl.md](speedgaming-etl.md) | One-way ETL of SG episodes into Wizzrobe `Match` rows; hybrid read-only (per-field + lifecycle guards); placeholder users for unresolved players |
...
| 2026-07-12 | **Wizzrobe succeeds SahasrahBot** (full replacement of its tournament/racing mission, not coexistence) |
...
| 2026-07-13 | **Wizzrobe owns the full racetime race-room lifecycle** ([Feature 5](racetime-room-lifecycle.md), added after the initial four): create → open → in-progress → finished/cancelled, for scheduled matches. **Rooms are auto-created on schedule** (opened ahead of `Match.scheduled_at`), not on demand. Reverses the earlier "race-room automation out of scope" decision and resolves the corresponding open question. |
| 2026-07-13 | SahasrahBot's **Async Tournament** is renamed **Async Qualifier** in Wizzrobe — its workflow is completely different from a tournament's and the name collision with `Tournament` would mislead |
| 2026-07-13 | SpeedGaming schedule data comes in via an **ETL/sync process** into Wizzrobe's own tables — *not* SahasrahBot's approach of live-querying the SG API per interaction |
...
| 2026-07-13 | **SG sync is strictly one-way** (SG → Wizzrobe, never back). For a `Match` created by the ETL, **the ETL-owned fields are read-only in Wizzrobe** — enforced at the service layer, not just convention. See [Feature 4](speedgaming-etl.md). |
| 2026-07-13 | **Async Qualifier execution is web-first, decided (not open).** SahasrahBot's Discord-thread-driven run execution (slash command → private thread → buttons) is **dropped**, not ported, not offered as a config toggle. Drawing a permalink, timing a run, submitting time/VoD, and review all happen on Wizzrobe web pages; Discord is notification-only via the existing DM queue. Live races still need a racetime.gg bot connection for synchronous group races, but that's separate from the core async execution loop. |
...
| 2026-07-15 | **Multiple racetime categories per tenant.** The `RacetimeBotTenant` junction is many-to-many: a tenant may be authorized for several categories (a community running alttpr + smz3 + sm), and each tournament selects the bot for its game via `Tournament → RacetimeBot`, constrained to the tenant's authorized, game-matching categories. (Already supported by the junction; made explicit.) |
...
Wizzrobe runs **Wizzrobe**: an *on-site* event where players are physically
...
**Wizzrobe is the designated successor to SahasrahBot.** The end state is that
SahasrahBot's tournament/racing mission retires and Wizzrobe is the single platform
for both Wizzrobe's on-site events and the online community's tournaments. The
...
- **SahasrahBot was refactored to Wizzrobe's exact architecture**
  (`alttprbot/{models,repositories,services,presentation}`), so ports are
  translations into existing layers, not rewrites.
- **Wizzrobe already has the supporting substrate**: multitenancy (each SahasrahBot
...
writes and ships code. In Wizzrobe, **a community defines tournament behavior as
...
**These are additive — they sit on top of Wizzrobe's existing Tournament/Match
...
| # | Feature | SahasrahBot source | Wizzrobe disposition |
|---|---|---|---|
...
| 3 | **[Discord Events sync](discord-events-sync.md)** | `ScheduledEvents` model (`episode_id` ↔ `scheduled_event_id`) | Generalized auto-sync of Discord Scheduled Events from Wizzrobe schedule data, per tenant guild |
| 4 | **[SpeedGaming schedule ETL](speedgaming-etl.md)** | direct SG API queries keyed on `episode_id` throughout | **New approach**: periodic ETL into Wizzrobe tables; SG becomes an upstream data source, not a per-interaction dependency |
...
- **General Discord-community features** (reaction/voice roles, holy images,
  ...
  Wizzrobe); Wizzrobe does not absorb them.
...
- **Data (decided): computed-and-cached, not persisted.** The latest probe result
  per dependency lives in an in-memory cache (safe under the single-worker model),
  refreshed by the cadence worker; a restart simply re-probes (brief `unknown` until
  the first run). No DB model or migration. This trades away cross-restart history
  and trend charts — acceptable for a "is it up right now?" board — and can be
  promoted to a persisted `ExternalServiceHealth` model later if trends are wanted,
  with no change to the probe interface. (Note: `racetime_bot.status` *is* persisted
  on its own row regardless, since the bot connection writes it directly.)

## Succession & cutover

- **Each SahasrahBot community becomes a tenant**; provisioning via the existing
  `/platform` surface. Wizzrobe is the first tenant.
- **The cutover gate is the five features**, evaluated per community: a community
  cuts over when the features *it uses* (from this list) are live in Wizzrobe.
...
- **Parallel run during transition**, each community's racing owned by exactly one
  ...
  category/bot identity transfers to Wizzrobe at the end, since
  [Feature 5](racetime-room-lifecycle.md) makes Wizzrobe the room owner.
