...
| `mcpserver/` | Remote MCP server at `/mcp` (peer of `api/`): read-only typed tools for Claude Desktop/Code over the same services, with its own OAuth 2.1 authorization server. Mounted before NiceGUI in `main.py`; its transport session manager runs for the app's lifetime via `session_lifespan()` in the lifespan | [features/mcp-server.md](features/mcp-server.md) |
