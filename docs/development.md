...
- **Global (tenant-agnostic):** users (people log in everywhere and hold per-tenant roles), the feature-flag groups (Default / Online Tournaments / Full Access), and the racetime bots. One of the users, **`super_admin`** ("Platform Owner"), holds only the global `SUPER_ADMIN` role and **no tenant role anywhere** — the fixture for `/platform` and for a platform admin's authority inside a community they have no grants in. No other seeded user is a super-admin: `staff_user` used to be, which made every staff-vs-super-admin difference untestable in dev.
...
**Idempotency:** everything uses `get_or_create`, so the script is safe to re-run — existing records are left unchanged. The exception is the fixtures whose whole point is a *specific* state that a migration also creates: the demo feature-flag groups and the two per-tenant flag overrides use `update_or_create`, because migration 31/32 seed their own versions (built from the flags that existed then) and a `get_or_create` would silently keep those instead — leaving the community-opted-out state, and any flag added after the migration, absent from the tier the seed claims to define. That also means match timestamps are *not* refreshed on re-run (matches are matched by title + tournament); to regenerate the relative scheduled times, delete the fixture matches (or reset the database) first.
...
| Path | Covers |
|---|---|
| `tests/api/` | REST API suites, one module per router family (`test_matches.py`, `test_volunteers.py`, `test_brackets.py`, …). Each mounts the API router on a minimal FastAPI app and calls it through an httpx `ASGITransport` client; `tests/api_helpers.py` holds the shared token/client setup. `test_reads.py`, `test_match_writes.py`, `test_admin_writes.py` and `test_content_and_config_writes.py` are the cross-router read/write suites; `test_error_paths.py` and `test_watchers_and_availability.py` fill in the remaining branches (403/404, read-only-token rejection, validation) |
| `tests/tenancy/` | Multitenancy: `test_tenant_context.py`, `test_tenant_middleware.py`, `test_tenant_urls.py`, `test_tenant_session.py` for the mechanics, plus every `*isolation*.py` leak test and the `test_leak_test_coverage.py` ratchet that keeps that set complete |
