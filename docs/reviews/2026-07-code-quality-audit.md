...
**Reconciled:** 2026-07-16 against `9f1f9e3` — see below.

## Reconciliation (2026-07-16, HEAD `9f1f9e3`)

Every finding was re-verified against the current tree. **No source changed between the audited commit `a402d19` and current `HEAD` `9f1f9e3`** — the only commits since are the two that landed this report itself (`git diff a402d19..HEAD` touches nothing outside `docs/reviews/`). So reconciliation here is an **accuracy pass**, not a fixed/stale sweep: each cited `file:line`, described behavior, and quantitative claim was re-checked against the code as it stands.

**Method.** A fan-out of nine per-section reviewers re-opened every cited location and re-counted every quantity with `ripgrep`, then any finding a reviewer wanted to downgrade was handed to an independent adversarial re-checker instructed to defend the original.

**Outcome — the report holds up.** Across ~90 verified checkpoints: **0 findings retracted, 0 false positives, 0 substantively wrong.** All three §1 correctness bugs, the "clean bill" claims (§2B no-repo-imports, §2C NiceGUI anti-patterns), and the §5 positives verified as written. The one finding a reviewer flagged as a false positive (§2D "Low") was **re-confirmed valid** on adversarial re-check — the reviewer had simply overlooked the `#### Low (abbreviated)` subsection; its citations (`random.seed(1234)` unrestored at `test_utils_coverage.py:360-362`, 11 `tenant_id=1` sites, status-only assertions at `test_api_router_gaps.py:176-182,383-402`) all check out.

What did need fixing was **counts and line anchors that drifted or were mis-stated** — 77 findings were accurate exactly as written; the rest have been corrected in place below (look for _[Reconciled: …]_ notes). The corrections that change a stated number or claim:

| Finding | Correction |
|---|---|
| **§3.7** | `scripts/seed_dev.py` is **790 lines, not 737** — making it the **largest module** (10 lines of headroom), so "largest is 789" was wrong. The other four sizes are exact. |
| **§2A.2** | Update-boilerplate is in **20 methods across 16 files**, not 19/15 — the list omitted `feature_flag_group_repository.py:40` (`user_repository.py` loop is at :172, not :171). |
| **§2A.1** | `build_authorize_url` is **not** byte-identical — it differs by its one-line docstring ("Twitch" vs "racetime"); only `_opt_str` is byte-identical. Mock identities also differ (Twitch carries `display_name`). |
| **§2A.4** | `effective_segments` is **not** byte-identical — player calls `PlayerAvailabilityService.covers` (:111) vs `VolunteerAvailabilityService.covers` (:119); the player repo also adds `has_any()`. |
| **§2A.5** | `_collect_notified_discord_ids` builds players/commentators/**trackers only — not watchers**; the twice-repeated send loop differs in its warning string (not strictly verbatim). |
| **§2A.name-uniqueness** | The create/rename pair is duplicated in **5 services, not 6** — `tenant_service` is the extracted counter-example (`_validate_slug`/`_validate_domain` with `exclude_id`), not an instance. |
| **§2B.1** | `seeds.py:38` is **not** a helper — it uses the inline `Preset.get_or_none`→404 shape at `:40-42`; the 12-router list should swap it for `users.py:29`, which is an actual helper. |
| **§2B.7** | Bare-list endpoints are **~49, not ~25** (only `matches` carries a limit param); the high-cardinality subset is smaller but the raw count is ~2×. |
| **§2D.3** | Slug counts: `tenant-b` ×9 (not 7), `b` ×5 (not 4), **~27 sites across 23 files** (`beta` ×11, `community` ×2 correct). |
| **§2D.4** | 21 `app` fixtures, but only **19 wrap `build_api_app()`** — the 2 exceptions are the same hand-built apps flagged in §2D.8. |
| **§2D.6** | The common `utc(...)` signature is **`utc(y, mo, d, h=0, mi=0)`**, not `utc(hour, minute=0)`; `make_user`/`_user` appears in **31 modules, not 21**. |
| **§3.5** | Drop "unlike the Challonge/Twitch/Discord endpoints" — those upstream base URLs are **equally hardcoded**; only Wizzrobe's own OAuth callback URLs are env-overridable. |
| **§2A.11 / §2A.18 / §5** | `can_admin_qualifier` isn't byte-identical (two extra branches); the §2A.18 git-history parenthetical is wrong (helper landed in PR8, inline copies in PR6); the §5 "strict-`xfail` convention" is **no longer enforced** (no `xfail_strict`, zero `xfail` markers remain). |

Assorted ±1–2 line anchors were nudged to the exact line in place. The remediation ordering in §4 and the executive summary are unaffected — every listed problem is real.
...
| 5 | Repository CRUD boilerplate stamped into ~16 repositories | **High (DRY)** | mixed | `application/repositories/` |
...
1. A `TenantScopedRepository[T]` base + a `require_found(obj, label)` service helper (touches the most files: ~16 repos, 40+ inline load-or-raise blocks).
...
- Clients: `application/utils/twitch_client.py` vs `application/utils/racetime_client.py` — `_opt_str` is byte-identical and `build_authorize_url` differs only by its one-line docstring ("Twitch" vs "racetime") (`twitch_client.py:31-45` vs `racetime_client.py:36-49`); `exchange_code` identical modulo the error class (`:54-70` vs `:59-75`); `get_me` shares the parse/raise scaffold including the same inline `import json as _json` quirk (`:81-90` vs `:80-91`); the mock scaffolding (`_MOCK_IDENTITIES` + `MockXClient`) is identical (`:106-127` vs `:105-126`), though the mock identity data diverges (Twitch entries carry a `display_name` racetime lacks). _[Reconciled: `build_authorize_url` and the mock identities are near-identical, not byte-identical.]_
...
`player_availability_service.py:18-116` vs `volunteer_availability_service.py:21-124`: `set_windows`, `clear`, `availability_map`, `covers`, `effective_segments` are identical except the model class, audit action, and the volunteer opt-in check — `effective_segments` differs only in its one self-class reference (`PlayerAvailabilityService.covers` at `:111` vs `VolunteerAvailabilityService.covers` at `:119`). The repos are identical except the model name and a `has_any()` method the player repo adds. _[Reconciled: `effective_segments` is identical-modulo-the-self-class-ref, not byte-identical; player span is 18-116.]_ The pure algorithms (`covers`, `effective_segments`) will drift silently if a precedence bug is fixed in only one. *Extraction:* a shared `availability_windows.py` (pure functions over a `starts_at/ends_at/status` protocol) + a generic `AvailabilityRepository[T]`.

**[pre-existing] 2A.5 — Recipient-collection loops are triplicated in `MatchScheduleService` (~90 lines).**
`notify_match_participants` (`:303-334`), `notify_match_crew` (`:339-384`), and `_collect_notified_discord_ids` (`:541-559`) each build the players/commentators/trackers set with the same filter+prefetch+opt-in loops (`_collect_notified_discord_ids` stops at trackers — it does **not** add watchers); the send loop is near-duplicated twice (`:326-334` vs `:374-382`), differing only in its warning-log string. _[Reconciled: watchers are not in the third method; the two send loops differ in one log string.]_ These are also raw ORM reads that belong in a repository. *Extraction:* one `collect_match_recipients(...)` helper + one `_send_dms(...)` loop.
...
- **[new] 2A.11 — `AuthService` permission ladder written 3×** (cascade bodies at `auth_service.py:220-226,232-238,250-257`) — the same `is_system → is_super_admin → is_staff → has_role` cascade; the third (`can_admin_qualifier`) appends two extra branches after the shared ladder rather than being byte-identical. *Extraction:* `_system_admin_staff_or(user, role)`.
...
- **[new] 2A.13 — Sorted-JSON→sha256 `_content_hash` duplicated** (`speedgaming_etl_service.py:457-461` vs `discord_event_reconciler_service.py:258-267`); `_opt_str` defined 3× (`racetime_client.py:48`, `twitch_client.py:43`, `speedgaming_etl_service.py:464`). *Extraction:* `stable_content_hash(obj)` in utils.
...
`User.preferred_name` re-implemented (`async_qualifier_rules.py:47`, `discord_event_reconciler_service.py:235` — both differ slightly from the property, so not exact copies); optimistic-concurrency check ×2 (`user_service.py:254-257,288-291`); M2M grant/revoke quadruplets (`tournament_service.py:255-301`, `async_qualifier_service.py:208-228`); name-uniqueness create/rename pair in **5** services (not 6 — `tenant_service` already factors the check into `_validate_slug`/`_validate_domain` with `exclude_id`, so it is the counter-example, not an instance); racetime bot status writers ×3 (`racetime_bot_service.py:211-272`); `_audit_and_emit` exists in exactly one service (`race_room_service.py:283-298`) while siblings hand-roll audit-then-publish — *promote to `AuditService.write_and_publish`*; `EventType.ALL` (`events/event_types.py:90-106`) hand-repeats every member (drift hazard — build it programmatically); `to_utc_aware(dt)` belongs in `utils/timezone.py` (naive-datetime fix-up copied at `service_health_service.py:271,449`, `challonge_service.py:68`, `speedgaming_etl_service.py:453`).
...
- **[mostly new] 2B.1 — `_load_*_or_404` helper copy-pasted into 12 routers** (plus 3 verbatim copies of `_load_user_or_404` and ~12 inline equivalents). The identical 4-line `Model.get_or_none(id=..., tenant_id=require_tenant_id())` → `HTTPException(404)` shape recurs in `async_qualifiers.py:44`, `async_qualifier_live_races.py:31`, `race_rooms.py:31`, `race_room_profiles.py:20`, `speedgaming.py:28`, `discord_events.py:27`, `presets.py:25`, `racetime_bots.py:31`, `users.py:29`, `match_actions.py:31`, `tournament_actions.py:22`, `stream_room_actions.py:15`. Helper names even drift (`_load_or_404` vs `_load_<entity>_or_404`). _[Reconciled: the enumeration originally listed `seeds.py:38`, which has no helper — its inline `Preset.get_or_none`→404 is at `seeds.py:40-42` and belongs in the "~12 inline equivalents" bucket; the real 12th helper is `users.py:29`.]_
...
- **[mixed] 2B.7 — Response-envelope inconsistency** — one paginated envelope (`AuditLogPage`), one limit-only bare list (`matches.py`), and ~49 unbounded bare-list endpoints (`response_model=List[...]` occurs 51× and only `matches.get_matches` carries a limit param; the genuinely high-cardinality subset is smaller, since many are small per-parent sub-collections). *Fix:* a shared `Page[T]` + `pagination` dependency for the high-cardinality collections. _[Reconciled: ~49 bare-list endpoints, not ~25.]_
...
- **[new] 2C.1 — Admin CRUD-tab scaffolding cloned into 4 new tabs.** `admin_presets.py`, `admin_racetime.py`, `admin_speedgaming.py`, and `admin_discord_events.py` are structural clones of `admin_webhooks.py`/`admin_discord_roles.py`, repeating eight blocks that should be shared components — the codebase already proves the pattern with `TournamentTableView`/`UserTableView`/`build_refreshable_board`:
  ...
  2. **`_current()` actor helper** — 7 copies + 74 raw call sites of the same one-liner across 43 files. *Extract `current_actor()`.*
...
- **[new] 2C.6 — Profile "link account" sections ×3** — `challonge_link_section.py` (48 lines), `twitch_link_section.py:13-47`, `racetime_link_section.py:13-47` (47 lines each) are identical except service/attrs/copy. *Extraction:* `render_link_section(...)`.
- **[pre-existing] 2C.7 — Equipment table duplicated across admin and home** — status-badge slot, mobile grid card, checkout/checkin handlers, and `_STATUS_LABELS` (defined 3× — `admin_equipment.py`, `home_tabs/equipment.py`, and `pages/equipment.py`) are duplicated. *Extraction:* `theme/tables/equipment.py`.
...
Leaderboard table + `_fmt` duplicated (`admin_qualifiers.py:459-475` vs `qualifiers.py:229-249`); one-time-secret reveal dialog ×2; dead duplicate table slots (`admin_settings.py:129-169`, 15 dead lines); `TournamentTableView`/`UserTableView` share an unextracted `BaseTableView` shape; a racy sync `@ui.refreshable` that kicks a background render (`triforce_texts.py:75-129`); a dead `schedule()` helper (`reports/shared.py:223-225`); a module-level service singleton (`platform.py:23`; the `qualifiers.py:56` instance is a `create()`-closure local, not module-level — verified stateless, so drift-not-bug); a captured-but-unused `client` param (`admin_discord_events.py:124`) and a background dialog with no client capture (`admin_speedgaming.py:183,209`).
...
*Verdict up front:* the suite is stronger than a glance suggests — the `*_coverage.py` modules are real behavior tests (ordering, upsert transitions, concurrency via `Event`/`Queue.join` rather than sleeps), the new API modules uniformly cover the 401 / 403-role / 403-read-only / 404 matrix plus tenant isolation, and `api_helpers.py` is genuinely reused across 21 test modules. The problems are fixture duplication and a handful of specific hazards.
...
- **[new] 2D.3 — Two-tenant setup duplicated across ~27 sites in 23 files, with four drifting slug conventions** (`tenant-b` ×9, `beta` ×11, `b` ×5, `community` ×2) plus a private `_tenants` helper re-defined in 4 modules and scattered `two_tenants`/`two_tenant_api`/`tenants_with_staff` fixtures. *Fix:* one parametrizable `two_tenants` fixture in `conftest.py`. _[Reconciled: `tenant-b` ×9 and `b` ×5, ~27 sites/23 files.]_

#### Medium

- **[pre-existing] 2D.4 — `app` fixture re-pasted 21×** — 19 of the 21 wrap `api_helpers.build_api_app()`; the other two (`test_api_matches.py`, `test_api_tokens.py`) hand-build the app, the same pair flagged in 2D.8.
- **[pre-existing] 2D.5 — `bypass_auth` fixture in 13 near-identical copies** across `tests/services/`.
- **[new] 2D.6 — `def utc(...)` re-defined in 10 modules — one with an incompatible signature** (`test_volunteer_services_coverage.py:37` is `utc(hour, minute=0, day=4)`; every other copy is `utc(y, mo, d, h=0, mi=0)`, so a copy-paste between them silently produces wrong dates — a `(y, mo, d, h, mi)` call lands as `hour=y`, `minute=mo`). `make_user`/`_user` is re-defined in 31 modules with drifting signatures. *Fix:* `tests/factories.py`. _[Reconciled: the majority signature is `utc(y, mo, d, h=0, mi=0)`, not `utc(hour, minute=0)`; `make_user`/`_user` spans 31 modules.]_
...
`RACETIME_BASE = 'https://racetime.gg'` (`racetime_client.py:22`) and `SPEEDGAMING_BASE = 'https://speedgaming.org'` (`speedgaming_client.py:24`) are hardcoded, not env-overridable — as are the Challonge, Twitch, and Discord API base URLs (only Wizzrobe's own OAuth callback URLs like `DISCORD_CONNECT_REDIRECT_URL` are env-overridable); only relevant for staging-against-staging. _[Reconciled: the upstream base URLs are uniformly hardcoded — the original "unlike the Challonge/Twitch/Discord endpoints" contrast was wrong.]_ Worker cadences and timeouts are named module constants (acceptable). `seedgen_service.py:152,168` embeds full randomizer-flag URLs/strings as inline literals (preset-shaped data in code).
...
- **[low] `docs/current-state.md`** still labels PRs 0–10 (rows 55–65) and the REST expansion (row 66) "In progress | this PR" though all are merged.
...
No module exceeds the 800-line guideline (largest is `scripts/seed_dev.py` at 790 — 10 lines of headroom). Several 700+ modules grew this week and bear watching: `scripts/seed_dev.py` (790), `match_service.py` (789), `async_qualifier_service.py` (749, already post-split), `theme/dialog/match_dialog.py` (739), `discord_service.py` (721) — plus one 700+ test module, `tests/services/test_match_service_coverage.py` (788). _[Reconciled: `seed_dev.py` is 790 lines, not 737, making it the largest module (the earlier "largest is 789" was wrong).]_
...
- **Drift guards already in place:** `tests/services/test_event_audit_parity.py` turns a missing `EventType`-for-`AuditAction` into a red test; the conftest `db` fixture's tenant auto-stamp shim. _[Reconciled: the "strict-`xfail` convention" was dropped from this list — there is no `xfail_strict` in the pytest config and zero `xfail` markers remain in `tests/` (the last one was removed by PR #96), so nothing currently enforces it.]_
