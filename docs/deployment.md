...
| `REDIRECT_URL` | no | `{BASE_URL}/oauth/callback` | `pages/auth.py` | Discord OAuth callback on the platform host, built **at request time**. One registered URI on `PLATFORM_HOST` serves every path-mode tenant (the tenant return path is carried in the session, not the URL). In host mode the callback is instead `https://<tenant.domain>/oauth/callback`, so **each custom domain must also be registered** as a redirect URI in the Discord Developer Portal. Override only for non-standard platform-host callbacks. |
