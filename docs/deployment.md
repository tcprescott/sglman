...
| _(randomizer API keys)_ | — | — | `application/services/seedgen_service.py` | **Not environment variables.** Each community supplies its own on Admin → Randomizer Keys, stored per tenant in `randomizercredential`. See [seed-generation.md](reference/seed-generation.md#per-tenant-credentials). |
