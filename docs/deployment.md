...
| `BASE_URL` | no | `http://localhost:8000` | `application/utils/environment.py` | Public base URL of the app (trailing `/` stripped); used for deep links (QR codes, DM links) and to derive `PLATFORM_HOST` when it is unset. **Not** used to build the OAuth callback — that derives from `PLATFORM_HOST` so the two can diverge safely. Set this in production. |
| `PLATFORM_HOST` | no | host of `BASE_URL` | `application/utils/environment.py`, `middleware/tenant.py` | Bare `host[:port]` (no scheme/path) that serves the platform surface (landing page, `/platform`), every path-mode tenant at `/t/<slug>`, and the shared OAuth callbacks. A request whose `Host` matches a tenant's custom `domain` instead resolves to that tenant (host mode). Defaults to the network location of `BASE_URL`. **Logged at startup** — if that value is wrong, a configured custom domain silently renders the platform surface. See [multitenancy.md](features/multitenancy.md). |
| `HOST_OAUTH_MODE` | no | `local` | `application/utils/environment.py`, `pages/auth.py`, `application/services/oauth_handoff_service.py` | Custom-domain login strategy. `local` (Design A) completes Discord OAuth on the custom domain, so each domain's `https://<domain>/oauth/callback` must be a registered Discord redirect URI. `handoff` (Design B) runs OAuth on the platform host (one registered URI regardless of domain count) and hands the session to the custom domain via a short-lived, single-use, host-bound signed token (`STORAGE_SECRET`-signed; in-memory nonce store — single-worker). Switch to `handoff` when custom-domain count nears the provider's redirect-URI ceiling. See [host-based-routing-plan.md](plans/host-based-routing-plan.md) §4.2. |
| `TRUST_FORWARDED_HOST` | no | off | `application/utils/hostname.py` | Truthy values: `1`, `true`, `yes`, `on`. When set, host/scheme resolution honors `X-Forwarded-Host`/`-Proto` (last value only) — enable **only** behind a trusted reverse proxy that sets them. Required for custom-domain (host-mode) routing when the proxy rewrites `Host` instead of forwarding it verbatim. Host/scheme analogue of `TRUST_PROXY_FORWARDED_FOR` (client-IP); a proxy deployment usually wants both. |
| `REDIRECT_URL` | no | `{scheme}://{PLATFORM_HOST}/oauth/callback` | `pages/auth.py` | Discord OAuth callback on the platform host, built **at request time** from `PLATFORM_HOST` (the host that actually serves it — **not** `BASE_URL`, so a legacy value in `BASE_URL` can't misdirect the callback), with `https` forced for any non-`localhost` host. One registered URI on `PLATFORM_HOST` serves every path-mode tenant (the tenant return path is carried in the session, not the URL). In host mode the callback is instead `https://<tenant.domain>/oauth/callback`, so **each custom domain must also be registered** as a redirect URI in the Discord Developer Portal (unless `HOST_OAUTH_MODE=handoff`, which needs only the platform URI). Override only for non-standard platform-host callbacks. |
...
| `DISCORD_BOT_PERMISSIONS` | no | `268435456` (Manage Roles) | `application/services/discord/discord_link_service.py` | Permissions integer requested in the "Connect Discord server" bot-authorization flow. Default is the minimum needed to apply mapped roles; widen it (e.g. add Manage Server) if the bot needs more. |
| `DISCORD_CONNECT_REDIRECT_URL` | no | `{BASE_URL}/oauth/discord/connect/callback` | `application/services/discord/discord_link_service.py` | Callback for the bot-authorization (server-connect) flow, built at request time. One URI on `PLATFORM_HOST` serves every tenant; must match a redirect URI registered in the Discord Developer Portal. |
| `MOCK_DISCORD` | no | off | `application/utils/mocks/mock_discord.py` | Truthy values: `1`, `true`, `yes` (case-insensitive). Bypasses OAuth and stubs `DiscordService` — see [features/mock-discord.md](features/mock-discord.md). **Refused when `ENVIRONMENT=production`** (see below). |
...
| `DK64R_API_KEY` | no | — | `application/services/seedgen_service.py` | Required only for DK64 Randomizer seed generation (sent as the `X-API-Key` header to api.dk64rando.com; issued by the DK64 Randomizer team). Missing value raises `ValueError` at generation time, not at startup. The `dk64r` randomizer is additionally gated by the per-tenant `DK64_RANDOMIZER` feature flag (off by default). |
| `MOCK_SEEDGEN` | no | off | `application/utils/mocks/mock_seedgen.py` | Truthy values: `1`, `true`, `yes` (case-insensitive). Short-circuits `SeedGenerationService.generate_seed` (qualifier pool rolling and every other caller) to a fake `mock.seedgen.local` permalink instead of reaching a live randomizer — lets local dev / the browser-validation loop exercise the full seed-rolling flow with no randomizer credentials. **Refused when `ENVIRONMENT=production`** (raises `RuntimeError`). |
...
| `MOCK_CHALLONGE` | no | off | `application/utils/mocks/mock_challonge.py` | Truthy values: `1`, `true`, `yes` (case-insensitive). Swaps in a stub Challonge client/connection for local dev. **Refused when `ENVIRONMENT=production`** (raises `RuntimeError`). |
