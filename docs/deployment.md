...
| `MOCK_SEEDGEN` | no | off | `application/utils/mock_seedgen.py` | Truthy values: `1`, `true`, `yes` (case-insensitive). Short-circuits `SeedGenerationService.generate_seed` (qualifier pool rolling and every other caller) to a fake `mock.seedgen.local` permalink instead of reaching a live randomizer — lets local dev / the browser-validation loop exercise the full seed-rolling flow with no randomizer credentials. **Refused when `ENVIRONMENT=production`** (raises `RuntimeError`). |
