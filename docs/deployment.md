...
| `MOCK_RACETIME` | no | off | `application/utils/mock_racetime.py` | Truthy values: `1`, `true`, `yes` (case-insensitive). Records a fake verified racetime identity so local dev can click through link/unlink without a real OAuth app. **Refused when `ENVIRONMENT=production`** (raises `RuntimeError`). |
