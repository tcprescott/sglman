...
- [multitenancy-plan.md](plans/multitenancy-plan.md) — the original design for making Wizzrobe logically multitenant (`/t/<slug>` + custom-domain addressing, platform super-admin surface, one-bot-many-guilds Discord). **Implemented** — see [features/multitenancy.md](features/multitenancy.md) for the shipped system; this is the design record.
- [brackets-plan.md](plans/brackets-plan.md) — native tournament bracket management (single/double elimination, Swiss, round robin, multi-stage chains e.g. groups → playoff) replacing Challonge-mirrored brackets: engine composition (ported MIT smwa elimination code + `swisspair` + in-house RR behind one strategy interface), generate-then-persist architecture, correctness harness, and the 2026-07 library research record. **Implemented** — see [features/brackets.md](features/brackets.md) for the shipped system; this is retained as the design record.
- [bracket-match-integration-plan.md](plans/bracket-match-integration-plan.md) — the bracket ↔ scheduled-match seam: one derived status vocabulary, live match state on the bracket, the cancel/delete slot release, the settled-result correction rule, series context on the match surfaces, and the matchup-ready DM. **Implemented** — see [features/brackets.md](features/brackets.md); retained as the design record.
- [bracket-ui-plan.md](plans/bracket-ui-plan.md) — **proposed** redesign of the bracket/Swiss/groups presentation layer to the canonical Challonge/start.gg/Liquipedia visual grammar (connector-lined match cards with seeds/avatars/scores, round headers with best-of and times, tiebreaker columns, advancement tinting), with the 2026-07 bracket-UI research record (layout techniques, library survey).
- [single-worker-escape-plan.md](plans/single-worker-escape-plan.md) — **umbrella plan** for the `--workers 1` constraint and the 500-concurrent-websocket capacity target: measured capacity numbers, a verified inventory of all sixteen process-global singletons and what each does at N>1, and why `uvicorn --workers N` is unreachable for a NiceGUI app. **Lazy tab rendering is shipped and the 500-connection target is met on one worker**; the four docs below carry the remaining work. Supersedes roadmap item 22 of the [project-structure review](reviews/2026-07-project-structure-review.md).
  - [render-cost-plan.md](plans/render-cost-plan.md) — **proposed** continuation of the admission-rate work (match-table element count, the N+1 query fan-out, a queries-per-render regression guard). Highest value per unit effort: it multiplies across every replica added later.
  - [singleton-ownership-plan.md](plans/singleton-ownership-plan.md) — **proposed** Postgres advisory-lock leader election so a second process degrades loudly instead of duplicating DMs, racetime rooms, and syncs. Explains why item 22's "fail fast if `workers>1`" cannot be implemented as written. Prerequisite for the two below.
  - [web-worker-split-plan.md](plans/web-worker-split-plan.md) — **proposed** `web`/`worker` role split (availability, not capacity) plus the Postgres `LISTEN`/`NOTIFY` seam that keeps live UI refresh working across the boundary.
  - [web-replicas-plan.md](plans/web-replicas-plan.md) — **proposed, not currently needed**: N web replicas behind sticky sessions + Redis. Build only on a measured admission-rate shortfall.
- [host-based-routing-plan.md](plans/host-based-routing-plan.md) — custom-`Host` tenant resolution and how login survives per-host session cookies (the host-local-OAuth vs signed-handoff fork). **Phase 1 and Phase 2 are implemented** (see [features/multitenancy.md](features/multitenancy.md)), including host-native secondary-provider identity-link OAuth via the same signed handoff; retained as the design record. Only the privileged token-holding flows (Challonge STAFF service-connect, Discord server-connect) remain main-site-only.
...
| `application/utils/` — 30 modules, incl. `clients/` (third-party HTTP clients) and `mocks/` (`MOCK_*` flags + stand-ins) | [reference/services.md](reference/services.md); timezone.py → [timezone-handling.md](timezone-handling.md) |
...
| `tests/` — `services/`, `api/`, `tenancy/`, `theme/`, and the root utility suites | [development.md](development.md) |
