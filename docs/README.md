...
- [single-worker-escape-plan.md](plans/single-worker-escape-plan.md) — **proposed** path out of the `--workers 1` constraint: a verified inventory of all sixteen process-global singletons and what each does at N>1, why `uvicorn --workers N` is unreachable for a NiceGUI app, and the retargeted plan (advisory-lock leader election → Postgres `LISTEN`/`NOTIFY` → a web/worker process split). Supersedes roadmap item 22 of the [project-structure review](reviews/2026-07-project-structure-review.md).
