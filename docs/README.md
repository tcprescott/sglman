...
- [host-based-routing-plan.md](host-based-routing-plan.md) — resolving a tenant from its custom `Host` (not just `/t/<slug>`), and how login survives per-host session cookies (the host-local-OAuth vs signed-handoff fork). **Phase 1 (Design A) and most of Phase 2 are implemented** — see [features/multitenancy.md](features/multitenancy.md); the doc is retained as the design record. Shipped in Phase 2: the flag-gated Design B signed-handoff (`HOST_OAUTH_MODE=handoff`) and the `tenant_base_url()` deep-link helper. Still deferred: host-native secondary providers and the forced `/t/<slug>`→domain 301.
