...
- [host-based-routing-plan.md](host-based-routing-plan.md) — concrete follow-through for the one deferred piece of multitenancy: resolving a tenant from its custom `Host` (not just `/t/<slug>`), and how login survives per-host session cookies (the host-local-OAuth vs signed-handoff fork). Design only; written for adversarial review before implementation.
