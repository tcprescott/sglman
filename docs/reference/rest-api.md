...
- **Lifecycle:** `POST /brackets/{id}/start` (generate the match graph) · `POST /brackets/matches/{match_id}/result` (`{winner_entry_id, entry1_score?, entry2_score?, forfeit?}` — optional set scores; the winner must have the strictly-higher score unless `forfeit`) · `POST /brackets/{id}/complete` (finalize the stage) · `POST /brackets/advance-stage?tournament_id=` (`{from_stage_order}` — seed the next stage from the prior stage's ranks). `BracketMatchResponse` surfaces `entry1_score` / `entry2_score` / `forfeit`, plus `best_of` and a nested `games` list.
