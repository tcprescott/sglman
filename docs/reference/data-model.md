...
- **Column-fit guards** — [`models/column_guards.py`](../../models/column_guards.py) attaches two validators to every field of every model at import of `models/__init__.py`: a string longer than its `CharField(max_length=N)`, or an integer outside its column's declared range (`INT` is 32-bit; `BIGINT`/`SMALLINT` read their own bounds off `Field.constraints`), raises `FieldValueError` — **a plain `ValueError`**, so it lands as a `ui.notify` on the web, a `400` over REST, and an `invalid_request` tool error over MCP. Without it these are production-only 500s: Tortoise's own over-length error is a `tortoise.exceptions.ValidationError` (not a `ValueError`, so every `except ValueError` misses it), integer range is not checked at all before `asyncpg` rejects it, and the test suite's SQLite backend enforces neither. The guard is a backstop, not a substitute for a service-layer bound that carries a meaningful message (e.g. `MAX_RUN_SECONDS` on a qualifier finish time).
...
- **Tenant-scoped (`tenant` FK, NOT NULL, `CASCADE`):** every other model —
  ...
  `BracketEntry`, `BracketMatch`, `BracketMatchGame`.
...
- **Per-tenant uniqueness** — formerly-global uniques became composite with
  ...
  `ChallongeApiUsage.period` → `(tenant, period)`;
...
| `COMPLETE` = `'complete'` | Winner recorded |
| `CANCELLED` = `'cancelled'` | Never played — the series clinched, or staff overrode the result |
...
| `value` | `TextField` | not null | **Plaintext at rest**, guarded at the service layer: never serialized into a listing, never written into an audit payload, write-only in the UI. Same contract as `RacetimeBot.client_secret`, `ChallongeConnection.access_token`, and `Webhook.secret` — if encryption at rest ever lands, these belong in the first batch together |
...
One game of a bracket match's best-of-N series, and **the scheduling seam**:
every game is its own scheduled `Match`, and a `Match` never backs more than one
game. A best-of-1 is a series with a single game, so there is one code path
rather than a special case. Rows are created **lazily, at schedule time** —
"game 3 of 3, not yet scheduled" is arithmetic from `best_of` minus the existing
rows, so changing `best_of` mid-series never orphans anything. `game_number` is
assigned by the service, never by a caller.

| Field | Type | Null / default | Notes |
|---|---|---|---|
| `tenant` | FK → `Tenant` | not null, `CASCADE` | `related_name='bracket_match_games'` |
...
| `RandomizerCredentialRepository` | [`randomizer_credential_repository.py`](../../application/repositories/randomizer_credential_repository.py) | `RandomizerCredential` | `list_all`, `get_by_natural_key`, `upsert`, `delete_by_natural_key`, `configured_pairs` (+ the `TenantScopedRepository` CRUD) |
| `AsyncQualifierRepository` (+ `Pool`/`Permalink`/`Run`/`LiveRace`/`ReviewNote` repos, same module) | [`async_qualifier_repository.py`](../../application/repositories/async_qualifier_repository.py) | `AsyncQualifier`, `AsyncQualifierPool`, `AsyncQualifierPermalink`, `AsyncQualifierRun`, `AsyncQualifierLiveRace`, `AsyncQualifierReviewNote` | Tenant-scoped/stamped. qualifier/pool/permalink/run CRUD + `list_active`, `get_with_relations`/`get_with_permalinks`; draw support `lock_user_for_draw` (SELECT … FOR UPDATE), `get_active_for_user`, `played_permalink_ids_for_user_in_pool`, `valid_run_counts_by_permalink_for_pool`; scoring/review `list_valid_for_qualifier`, `list_approved_finished_for_permalink`, `list_pending_review`; live races `get_by_racetime_slug`, `list_for_live_race` |
...
| `RaceRoomProfileRepository` | [`race_room_profile_repository.py`](../../application/repositories/race_room_profile_repository.py) | `RaceRoomProfile` | Standard tenant scoping. `list_all`, `get_by_id`, `get_by_name`, `create`, `update`, `delete` |
...
**Multitenancy and online tournaments (migrations 20–38).** The current head is **migration 38**. The later migrations, in order: **20** — the additive multitenancy migration (adds `Tenant`/`TenantMembership`, a `tenant` FK on ~33 models, backfills a `default` tenant, tightens to NOT NULL; see [features/multitenancy.md](../features/multitenancy.md)); **21** — online-tournament foundations (system user, the `PRESET_MANAGER`/`SYNC_ADMIN`/`QUALIFIER_ADMIN` roles, the hybrid-config substrate); **22** — user-managed `Preset`; **23** — racetime identity fields on `User`; **24** — `RacetimeBot`/`RacetimeBotTenant`/`RaceRoomProfile`/`RacetimeRoom`; **25** — `matchplayers.finish_time`; **26** — SpeedGaming ETL (placeholder `User`, `SpeedGamingEventLink`/`Episode`, the `Match` source marker); **27** — `DiscordScheduledEvent`; **28** — the five `AsyncQualifier*` tables; **29** — `AsyncQualifierLiveRace`; **30–31** — per-tenant feature flags (`TenantFeatureFlag`, then `FeatureFlagGroup` + `Tenant.feature_group`; see [features/feature-flags.md](../features/feature-flags.md)); **37** — `RandomizerCredential`; **38** — retires the `dk64_randomizer` flag rows that per-tenant credentials replaced.
