...
Lifecycle of a bracket stage (`Bracket.state`, `max_length=16`): `DRAFT` = `'draft'` (entrants enrolled/seeded; the engine has not run), `ACTIVE` = `'active'` (`start` has generated and persisted the match graph), `COMPLETE` = `'complete'` (every match resolved; each entry's `final_rank` written).
