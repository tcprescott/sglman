...
| Route | File | Auth |
|---|---|---|
| `/`, `/home/{section}` | [`pages/home.py`](../../pages/home.py) | Members only — `enforce_membership` renders the join page for everyone else, anonymous included. `/` = default section. On the bare platform host (no tenant) it renders the community picker instead |
...
| `/platform` | [`pages/platform.py`](../../pages/platform.py) | Super-admin only, **no tenant context**; tenant CRUD, with each other section in its own module — `platform_bots.py` (racetime bots), `platform_feature_groups.py` (tiers + a tenant's overrides), `platform_tenant_admins.py` (first staff member), shared bits in `platform_shared.py` (see [multitenancy.md](../features/multitenancy.md)) |
...
| [`equipment.py`](../../pages/home_tabs/equipment.py) | My Schedule → Equipment | The viewer's own checkouts, `EQUIPMENT` flag live. Browsing the register is admin-side; borrowing is the QR scan into `/equipment/<id>` |
...
- A `MatchTableView` (`admin_controls=False`, `player_discord_id` set). Columns: ID, Tournament, Scheduled At, State, Players, Stage, Generated Seed, Stream, Change, Settings, Watch, using the same read-only state/seed `extra_slots` as the Schedule tab.
...
One strip sits above the board, shown only to whoever can act on what it counts: the **review queue** (`access.confirm` — flagged / no-result / awaiting-confirmation, with a filter to them). `tournament_ids` narrows the board for a TA or crew coordinator, whose authority is per tournament; staff and the stream manager operate community-wide and get the whole board.
...
**`admin_tournaments_page()`** — `TournamentTableView` over `Tournament.all()`. Columns: Name, Description, Seed Generator, Active, Players/Match, Avg Match Duration, Max Match Duration, Staff Administered, Player Count (the ID column is hidden). **Add Tournament** renders only when `AuthService.is_staff(actor)` and opens `TournamentDialog`; name clicks edit, player-count clicks open `TournamentPlayersDialog`; both the edit dialog's save and its Danger-zone delete repaint the table. Listens for `selected_tab == 'Tournaments'` to refresh.
...
|---|---|---|---|
| [`_helpers.py`](../../theme/dialog/_helpers.py) | `form_dialog`, `dialog_header`, `dialog_actions`, `mobile_sheet`, `native_date_input`, `native_time_input`, `render_suggest_time_button`, `submit_on_enter` | The house dialog chrome every dialog builds on — keeps actions reachable on a phone. `render_suggest_time_button` is the "Suggest a time" button shared by every match-scheduling dialog (`UserMatchDialog`/`AdminMatchDialog`, `BracketScheduleDialog`, `ChallongeScheduleDialog`) — it fills Date/Time from `MatchSuggestionService.suggest_match_time` on click rather than pre-filling silently on open | (imported) |
| [`match_dialog.py`](../../theme/dialog/match_dialog.py) | `BaseMatchDialog`, `AdminMatchDialog`, `UserMatchDialog` | Create/edit matches | Admin schedule; home Event → Schedule and My Schedule → matches |
...
| [`cat_fact_dialog.py`](../../theme/dialog/cat_fact_dialog.py) | `CatFactDialog` | Easter egg — five taps on the header wordmark, or the Konami code. "Meow another" re-rolls (never to the fact already on screen); "See them all" opens [`/cat-facts`](#pages-and-routes) | `BaseLayout._render_header` |
...
The largest UI component, used by the home Schedule and Player tabs and the admin Schedule tab. Split across `match.py` (view), `match_filters.py` (the filter strip and its stored state), `match_slots.py` (Vue templates), `match_grid.py` (cards), and `match_handlers.py`.
