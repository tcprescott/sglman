...
| [`waiting.py`](../../theme/waiting.py) | `waiting_panel(message)` — spinner + message + a cat fact, for a wait the app cannot shorten. Returns the element so the caller removes it when the work lands. Used by `BaseLayout._build_tab` (deferred tab builds) and the two OAuth wait screens in [`pages/auth.py`](../../pages/auth.py), which otherwise sit on a blank page through the Discord round trip |
