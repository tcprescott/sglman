...
- A match scheduled by a bracket shows a "<stage> · Game N" link under the tournament name, into the (anonymous-readable) bracket view. Row field: `MatchDisplayService._bracket_ref`; slots: `TOURNAMENT_SLOT` (`match_slots.py`) and the mobile caption (`match_grid.py`); handler: `_handle_open_bracket`. It emits `open_bracket` rather than carrying an `href` so `ui.navigate.to` prepends the tenant `root_path`. Shared by every `MatchTableView`, so the admin schedule and player dashboard get it too.
