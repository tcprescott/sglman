...
- A match scheduled by a bracket shows a "<stage> · Game 2 of 3 · 1-0" link under the tournament name, into the (anonymous-readable) bracket view — the series context comes off the games the repository already prefetched, so it costs no extra query per row. (The round *name* is deliberately absent here: naming a round needs the stage's whole graph, a query the table must not pay per row; the Discord DMs, which fan out one at a time, do resolve it.) Row field: `MatchDisplayService._bracket_ref`; slots: `TOURNAMENT_SLOT` (`match_slots.py`) and the mobile caption (`match_grid.py`); handler: `_handle_open_bracket`. It emits `open_bracket` rather than carrying an `href` so `ui.navigate.to` prepends the tenant `root_path`. Shared by every `MatchTableView`, so the admin schedule and player dashboard get it too.
...
**Create bracket** opens a dialog (name, format select, stage order, and a "Format / advancement options" expansion for Swiss rounds, round-robin group count, and the advancement rule) → `create_bracket`. The **Manage** dialog additionally carries a **Round settings** editor (per-round best-of / scheduled time → `set_round_metadata`, editable in any state), and the **Results** dialog **embeds the shared bracket renderer** for elimination formats — staff click a match card to report/override via the same visual dialog the public page uses (`build_match_dialog`), supplementing the flat lists. All handlers catch service `ValueError`/`PermissionError` and surface them via `notify_error`; `wire_tab_refresh('Brackets', …)` refreshes the table on tab entry.
...
- **`/tournament/{tournament_id}/brackets`** (`bracket_index`) — a card per stage (name, "Stage N · Format", state badge) with a **View** button into the detail route. DRAFT stages are filtered out for non-staff (`visible_stages`).
- **`/brackets/{bracket_id}`** (`bracket_detail`) — a `@ui.refreshable` body that renders **by format** through the shared [`theme/brackets/`](#bracket-renderer-themebrackets) renderer: single/double elimination as a connector-lined 2-D bracket (winners+finals, losers below), swapped below `lt.md` for a per-round accordion with a **List / Bracket** toolbar toggle back to the 2-D view (exactly one renders at a time — see [brackets.md](../features/brackets.md#presentation--the-redesigned-bracket-view)); round robin as a responsive grid of group cards (tinted standings + crosstable + match list); Swiss as a standings panel plus round-tabbed pairings. Standings are computed live with `compute_standings` (the same pure helper the service uses). Clicking a match opens a detail dialog (staff get inline report/override with scores + forfeit); the view subscribes to `BRACKET_*` **and `MATCH_*`** events for live refresh (`theme/brackets/live.py`, the latter filtered on `tournament_id`) and installs a hover-run highlight. Every card carries its **derived live status** — a linked pulsing "LIVE" pill, or a Checked in / Awaiting result / Needs reschedule chip — from `BracketService.matchup_live_state`, resolved in one extra query for the whole field and shown to anonymous viewers too (see [brackets.md → Live match state](../features/brackets.md#live-match-state-on-the-cards)). A DRAFT / unstarted stage shows its seeded entrants instead of a graph — and is staff-only: for anyone else the route renders 'Bracket not found', since an unpublished stage's seeding is not public (`theme/brackets/visibility.py`).

#### Bracket renderer (`theme/brackets/`)

One in-house renderer, consumed by both the public page and the admin Results dialog (design record: [bracket-ui-plan.md](../plans/bracket-ui-plan.md)). `layout.py` is a **pure, ORM-free layout walker** (winner-link tree → absolute pixel placements + elbow connectors, robust to byes and the double-elim losers bracket; unit-tested in [`tests/theme/`](../../tests/theme/)). `cards.py` paints absolute-positioned match cards + sticky round headers; `tables.py` renders Swiss/group data tables (standings with tiebreaker columns + advancement tint/cut line, pairings, crosstable) as **NiceGUI elements, never `ui.html`** (entrant names are user-controlled). `visibility.py` is the pure staff-only-DRAFT rule shared by the public pages and the browse tab; `labels.py` holds the format/state chrome those surfaces and the admin format select share. `render.py` bundles whole-bracket helpers (`render_elimination`, mobile accordion, `build_context`, `detect_finals`) shared by both surfaces; `dialog.py` is the shared match report/override dialog; `live.py` bridges the event bus to a refresh. All color/size routes through `--bracket-*` custom properties in [`static/css/brackets.css`](../../static/css/brackets.css) (winner accent = the app `--q-primary`, dark/light aware), linked per-page via `add_head_html`. Those properties are declared on **`body`** — `ui.colors()` writes the tenant palette onto `body`, so a `:root` declaration would pick up Quasar's stock blue instead.
...
- **Bracket matchup** (optional) — shown only when `FeatureFlag.BRACKETS` is live. When the match already backs a game, a read-only **matchup panel** sits above the picker (`render_matchup_panel` → `BracketService.matchup_summary`): stage · round name, "Game 2 of 3 · Series 1-0 · Winner to Semifinals", both entrants with seeds, and a button into the bracket view — so staff editing a game can see *what it is for*. Below it the picker lists the selected tournament's OPEN matchups that still have a game slot free (`BracketService.list_linkable_matches`), repopulating with the tournament. Attaches a manually-scheduled match to the matchup it settles so its result advances the bracket. In edit mode it preselects the current link (added to the options by hand, since a linked matchup is no longer "linkable") and `(None)` unlinks. Lives in [`_match_bracket_link.py`](../../theme/dialog/_match_bracket_link.py).
