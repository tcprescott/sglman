...
- Challonge-linked tournaments (those with a `challonge_tournament_id`) don't use the manual opt-in: their checkbox is disabled and reflects bracket membership from `ChallongeService.participant_tournament_ids` (whether the player is mirrored as a participant). If the player hasn't linked their Challonge account (`user.challonge_user_id` unset) the row shows a "Link Challonge account" call to action instead. These tournaments are excluded from `tournament_checkboxes`; any existing manual enrollment for them is carried through unchanged on save so the registration sync never drops it.
