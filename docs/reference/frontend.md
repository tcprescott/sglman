...
**Data flow** — `refresh()` calls `MatchDisplayService.get_matches_for_display(tournament_ids, stage_ids, only_upcoming=False, user_discord_id, exclude_racetime)`, applies the state filter client-side, merges per-row `_watching` flags from `MatchWatcherService.list_watched_match_ids` and `_stream_volunteer` from `MatchStreamVolunteerService.list_volunteered_match_ids`, then applies `row_sort` (if given) and notifies `on_rows_changed`. `update_row_by_id(match_id)` re-fetches one row via `get_match_for_display` (deleting the row if the match is gone, preserving `_watching` and `_stream_volunteer`); `delete_row_by_id` removes a row from the UI only.
