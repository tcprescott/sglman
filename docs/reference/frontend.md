...
| `/equipment/qr-labels` | [`pages/equipment_labels.py`](../../pages/equipment_labels.py) | Staff / Equipment Manager (`EQUIPMENT` flag); printable Letter grid of asset QR labels (`?ids=…&cols=…`). Registered **before** `/equipment/{asset_id}` so the static path wins. |
...
- **Print QR labels** opens `QrLabelDialog` — a checklist of assets (all pre-selected) plus a column count. It opens [`/equipment/qr-labels`](../../pages/equipment_labels.py) in a new tab (via an absolute `tenant_url`) with the chosen `ids`, a printable 8.5×11 grid of each asset's QR + `#number` + name for the browser's Print.
...
| [`qr_label_dialog.py`](../../theme/dialog/qr_label_dialog.py) | `QrLabelDialog` | Pick assets + columns, then open the printable QR-label sheet in a new tab | Admin Equipment tab "Print QR labels" |
