...
- **Crew coverage assumes every streamed match needs a tracker.** `ReportsService.crew_coverage` flags `coverage_gap` when a stream candidate has no approved tracker (`reports_service.py:313`), and `AnalyticsService.tournament_health` only counts a candidate as covered with both an approved commentator *and* an approved tracker (`analytics_service.py:422`). In practice the tracker requirement varies by tournament, and nothing models that — `Tournament.config` is a closed schema (`messaging_templates` only). So a tournament that does not use trackers shows a permanent false coverage gap and a depressed health score. The dev seed now carries both crew shapes (two commentators plus a tracker, and two commentators alone) so the difference is visible; the fix is a per-tournament crew requirement.
