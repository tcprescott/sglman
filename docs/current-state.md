...
| Per-tenant feature flags | New | this PR | Two-tier per-tenant gating (super-admin grants *availability* on `/platform`; tenant STAFF *enable* in Admin → Features; live = available AND enabled), disabled by default. New `TenantFeatureFlag` model + `FeatureFlag` registry (`application/feature_flags.py`) + `FeatureFlagService`. Entry-layer gating: `@protected_page(feature=)`, admin/home tab checks, `require_feature` REST deps, worker skips. Initial flags: async qualifiers / racetime rooms / SpeedGaming ETL (ship dark) + challonge / equipment / volunteers / triforce (backfilled on for existing tenants). Migration 30 ([feature-flags.md](features/feature-flags.md)) |
...
application/repositories/ ← ORM queries (Tortoise)
  ↓ uses
models/                 ← Tortoise models (53) + enums (17: Role, RoleSource, FeatureFlag, MatchNotificationLevel, …), per-domain submodules re-exported from models/__init__.py
```
