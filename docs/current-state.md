...
- **The shared admin-CRUD extraction is dead code.** `theme/tables/admin_crud.py` exports `ServiceTableView`, `admin_page_container`, `refresh_icon_button`, `action_button` and `actions_slot`, and nothing outside the module imports any of them; `theme/tables/equipment.py` defines a competing `actions_slot`. Only `wire_tab_refresh` and `current_actor` are used. Two successive audits named this their highest-leverage fix and it still has no adopters — an extraction without a guardrail decays into an optional style choice.
- **`scripts/seed_dev.py` is 863 lines**, over the 800-line soft budget in `.claude/scripts/check_file_length.py`, and the only file in the tree over it. It was split 801 → 768 during the July remediation and has since regressed past its previous high.
