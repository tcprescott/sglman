...
- `@protected_page`: gains a membership gate (authenticated-but-not-a-member of this tenant → denied even with no role requirement). Add `current_membership()` helper. `get_user_from_discord_id()` stays global.
