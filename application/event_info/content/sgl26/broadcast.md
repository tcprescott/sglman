...
> **TODO:** stream resolution and bitrate targets, audio levels, the OBS scene
> collection or template in use (if any), delay/buffer settings, and the
> restream/relay setup for any tournament that mirrors to a second channel.
...
proctors use applies to you: a match in progress does not pause for a shift
change, so brief the incoming tech on what's live, what's flagged, and any
equipment you've been working around.
:::

## Contacts

> **TODO:** the broadcast lead or coordinator to ask about shifts and swaps,
> and who to escalate a production problem to that isn't a match-integrity
> issue (those still go to the on-call admin — see
> [Running a room](/event-info/proctoring#when-things-go-wrong)).

**If you genuinely cannot find anyone**, ping **@Staff** in the
[event Discord](https://discord.com/invite/T6qzxNJ). That is the backstop for
when nobody is reachable, not the normal route.
