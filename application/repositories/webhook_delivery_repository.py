...
class WebhookDeliveryRepository:
    ...
    async def create(self, **fields: Any) -> WebhookDelivery:
        return await WebhookDelivery.create(**fields)
    ...
    async def list_for_webhook(
        ...
        return (
            await WebhookDelivery.filter(webhook=webhook)
            .order_by('-created_at')
            .offset(offset)
            .limit(limit)
        )
    ...
    async def prune_older_than(self, cutoff: datetime) -> int:
        ...
        return await WebhookDelivery.filter(created_at__lt=cutoff).delete()
