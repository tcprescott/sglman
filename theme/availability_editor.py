...
window editor + effective-availability heatmap; they differ only in the backing
service (player vs. volunteer availability) and one line of help copy. This
module holds the single implementation so the two tabs stay in lockstep and the
heatmap palette is defined once (via the .avail-* token classes in styles.css).
"""
