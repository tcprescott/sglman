...
class TestCrewNeed:
    """How far a match is from staffed, per role.

    Coverage existed only as a report-time computation, which is why the only
    surface that knew about gaps was the one that could not act on them.
    """

    def _tournament(self, commentators=1, trackers=1):
        return SimpleNamespace(
            name='T', seed_generator=None, is_racetime_enabled=False,
            required_commentators=commentators, required_trackers=trackers,
        )

    def _crew(self, *approvals):
        return [
            SimpleNamespace(
                id=i, user=SimpleNamespace(preferred_name=f'P{i}', discord_id=None),
                approved=a, acknowledged_at=None, auto_acknowledged=False,
            )
            for i, a in enumerate(approvals)
        ]

    def test_an_unstaffed_match_reports_the_full_requirement(self, display_service):
        match = make_match(tournament=self._tournament(commentators=2, trackers=1))
        need = display_service._format_match_for_display(match)['crew_need']
        assert need == {'commentators': 2, 'trackers': 1}

    def test_an_unapproved_signup_has_not_covered_anything(self, display_service):
        """The decision that staffs the match is still outstanding."""
        match = make_match(
            tournament=self._tournament(commentators=1),
            commentators=self._crew(False),
        )
        assert display_service._format_match_for_display(match)['crew_need']['commentators'] == 1

    def test_an_approved_signup_covers_a_slot(self, display_service):
        match = make_match(
            tournament=self._tournament(commentators=2),
            commentators=self._crew(True, False),
        )
        assert display_service._format_match_for_display(match)['crew_need']['commentators'] == 1

    def test_a_fully_covered_role_needs_nobody(self, display_service):
        match = make_match(
            tournament=self._tournament(commentators=1),
            commentators=self._crew(True),
        )
        assert display_service._format_match_for_display(match)['crew_need']['commentators'] == 0

    def test_over_staffing_does_not_go_negative(self, display_service):
        match = make_match(
            tournament=self._tournament(commentators=1),
            commentators=self._crew(True, True, True),
        )
        assert display_service._format_match_for_display(match)['crew_need']['commentators'] == 0

    def test_a_role_the_tournament_does_not_use_needs_nobody(self, display_service):
        match = make_match(tournament=self._tournament(commentators=0, trackers=0))
        need = display_service._format_match_for_display(match)['crew_need']
        assert need == {'commentators': 0, 'trackers': 0}

    def test_a_match_with_no_tournament_needs_nobody(self, display_service):
        match = make_match(tournament=None)
        need = display_service._format_match_for_display(match)['crew_need']
        assert need == {'commentators': 0, 'trackers': 0}
